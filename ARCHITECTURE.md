...
| 항목 | 용도 | 필요 시점 | 상태 |
|---|---|---|---|
| Supabase 계정 + 프로젝트 | DB(Postgres+pgvector) 접속 정보 | 개발 순서 1단계 | 확인 필요 |
| Google AI Studio Gemini API 키 | 임베딩·답변 생성 | 개발 순서 3단계 | 확인 필요 |
| Render 계정 (GitHub 연동) | 크롤러(Cron Job), FastAPI 배포 | 개발 순서 4단계 이후 | 확인 필요 |
| Vercel 계정 (GitHub 연동) | Next.js 프론트엔드 배포 | 개발 순서 5단계 이후 | 확인 필요 |

계정 발급 자체는 개인 가입이 필요해 대신 처리할 수 없음. 발급 후 연결 정보(URL, key)는 코드에 직접 커밋하지 않고 `.env`(로컬)와 각 플랫폼의 환경변수 설정으로 관리.

### 결정된 운영 방침

- **학교 승인 절차**: 테스트 버전을 만들어 충분히 검증한 뒤, 학교 측(정보전산원 등 규정집 담당 부서) 승인 여부를 확인 — 정식 승인 전까지는 검색엔진 노출 차단(`robots.txt` disallow, `noindex` 메타태그)과 URL 비공개 공유로 운영 (프론트엔드 구현 시 기본 적용)
- **도메인**: 초기엔 Vercel 기본 서브도메인(`*.vercel.app`)으로 시작, 커스텀 도메인은 추후 검토
- **저장소 공개 범위**: `yshwkang/RuleMoa` Public 전환 완료 (커밋 이력에 자격증명 노출 없음을 확인함)
...
5. Next.js 프론트엔드 (입력 폼 + 응답 렌더링, 테스트 기간 noindex 적용)
6. 조항별 뷰페이지 (ISR)
